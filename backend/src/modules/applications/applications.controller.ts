...
export class ApplicationsController {
  ...
  forJob(@CurrentUser() user: RequestUser, @Param("jobId") jobId: string) {
    return this.applications.jobApplications(user.id, jobId);
  }

  @Roles(RoleCode.PARENT)
  @Post("applications/:id/accept")
  accept(@CurrentUser() user: RequestUser, @Param("id") id: string, @Body() dto: HandleApplicationDto) {
    return this.applications.accept(user.id, id, dto.note);
  }

  @Roles(RoleCode.PARENT)
  @Post("applications/:id/reject")
  reject(@CurrentUser() user: RequestUser, @Param("id") id: string, @Body() dto: HandleApplicationDto) {
