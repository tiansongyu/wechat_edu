...
PostgreSQL 是资料、发布、收藏、报名、预约、评价、通知、聊天、用户偏好和审核记录的唯一真相源。小程序页面只消费 API 返回的数据，本地存储仅用于 `accessToken`、`refreshToken`、`activeRole` 和稳定 `deviceId`，不提供模拟业务数据或离线成功回退。
...
- `appointments`：教师确认预约、家长和老师分别确认完课、双方取消或发起争议的履约状态机。
- `reviews`：真实预约互评、角色隔离聚合、幂等提交、匿名字段投影和评价资格判断。
- `communications`：通知、会话未读数、聊天已读和消息幂等。
- `preferences`：数据库中的接单通知、聊天通知和隐私偏好。
- `platform`：从 `system_settings` 白名单投影品牌文案，并实时统计可用老师、已发布信息、已完成合作和有效评价。
...
`appointments` 使用 `PENDING → CONFIRMED → COMPLETED` 主路径，也允许参与双方在规则允许的阶段取消或发起争议。教师先确认预约；进入 `CONFIRMED` 后，家长和老师分别写入自己的完课确认时间，第二方确认才会原子进入 `COMPLETED`。历史单方完成记录没有两项确认时间，因此不会自动获得评价资格。取消和争议必须填写原因。每次状态变化都写入 `statusNote`、`handledAt`、`version`、Outbox 和审计日志。
...
- Worker 领取事件时写入 `claimedAt`；进程异常退出后，超过租约的 `PROCESSING` 事件可重新领取。
...
accounts ─ account_roles ─ roles
   ...
   ├─ job_posts ─ applications ─ appointments ─ reviews
   ├─ favorites
   ├─ user_preferences
   ├─ refresh_sessions
   ├─ notifications
...
`GET /api/v1/platform/overview` 需要登录，并且只返回 `platform.public` 中经过校验的 `brand` 与 `trustHighlights` 字段；配置缺失或格式错误时返回明确的服务不可用错误，不使用前端假文案兜底。平台指标直接统计数据库中的有效记录：老师必须账号可用且资料已通过审核，发布信息的发布者必须可用，已完成合作双方账号也必须可用；公开评价必须仍为 `PUBLISHED`、来自已完成预约且评价双方账号可用。迁移和种子脚本均仅在配置不存在时写入默认值，不覆盖运营已维护的内容。
...
评价只允许由双方已确认完成且未处于争议状态的预约参与者创建。评价人、被评价人和双方角色均由服务端从预约关系推导；数据库唯一约束 `(appointmentId, reviewerId)` 与请求幂等记录共同阻止重复评价。家长身份和老师身份的聚合永不混算，公开明细不返回评价人账号标识，有效评价少于三条时不展示平均星级。
