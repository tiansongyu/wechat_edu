...
- 同一微信账号的家长/老师双角色切换，统一确认且不会在切换后自动执行业务命令
...
- 被评价人可按当前身份举报评价并持续查看处理进度，管理员可隐藏/恢复评价、判定违规或无违规
- 数据库驱动的平台品牌、信任说明与有效老师/发布/合作/评价实时概览
- 报名及接受/拒绝/取消、预约确认/完成/取消/争议均使用稳定幂等命令，配合唯一约束、串行化事务、名额锁定及操作原因留痕
- 数据库用户偏好、通知、会话未读数、聊天已读、幂等消息和 Outbox 异步事件
- MinIO 私有文件上传签名
- Vue 管理后台：看板、用户、教师审核、发布审核、报名/预约履约、评价举报治理、审计日志
...
node tests/reviews-e2e.mjs   # 验证双方完课、互评权限、匿名聚合、争议排除和举报治理闭环
