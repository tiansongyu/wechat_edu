...
async function load() { loading.value = true; try { const { data } = await api.get("/audit-logs", { params: query }); items.value = data.items; total.value = data.total; } catch (error) { ElMessage.error(getApiErrorMessage(error, "审计日志加载失败")); } finally { loading.value = false; } }
onMounted(load);
</script>

<template>
  <section class="content-card table-card">
    <div class="card-head"><div><h3>操作审计日志</h3><p>管理员与业务状态变更永久留痕</p></div><el-button :loading="loading" @click="load">刷新</el-button></div>
